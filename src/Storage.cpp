...
namespace ECE141 {
  ...
  std::string Storage::getIndexPath(const std::string &aTableName) {
    std::ostringstream theStream;
    theStream << Storage::getDefaultStoragePath() << "/" << aTableName << ".txt";
    return theStream.str();
  }
  ...
  std::vector<std::string> Storage::GetEntitiesName(){
    std::vector<std::string> names;
    for(unsigned int i=0; i<toc.entities.header.count; i++){
      names.push_back(toc.entities.items[i].name);
    }
    return names;
  }
  ...
  StatusResult Storage::writeBlock(int aBlockNumber, Block &aBlock) {
    ...
    if(theResult) BlockCache.put(aBlockNumber, aBlock);
    return theResult;
  }
  ...
  Block& LRUCache::get(uint32_t key) {
    update(hashmap[key]);
    return hashmap[key]->theBlock;
  }
  
  void LRUCache::put(uint32_t key, Block& aBlock) {
    if(hashmap.count(key)){
      update(hashmap[key]);
      hashmap[key]->theBlock = aBlock;
    }
    else{
      DoubleLinked *newnode = new DoubleLinked(key,aBlock);
      hashmap[key] = newnode;
      addNode(newnode);
      cur_size ++;
      if(cur_size > size){
  ...
  bool LRUCache::contains(uint32_t blockNum){
    return hashmap.count(blockNum);
  }
